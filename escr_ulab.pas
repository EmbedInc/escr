...
module escr_ulab;
define escr_ulab_get;
%include 'escr2.ins.pas';
{
********************************************************************************
*
*   Subroutine ESCR_ULAB_GET (E, NAME, STR_P)
*
*   Get the expansion of the unique label with generic name NAME.  STR_P is
*   returned pointing to the expansion string.
*
*   If the unique label does not previously exist, the expansion will be created
*   first, using the globally unique sequential number ULABN.  Each ULABN value
*   is only used to create one unique label.
*
*   The unique label context of the innermost execution block that has one
*   is used.
}
...
var
  ...
  fname: string_var80_t;               {full label name}
  tk: string_var32_t;                  {scratch token}
  ex_p: escr_exblock_p_t;              {pointer to execution block with labels table}
  pos: string_hash_pos_t;              {position within unique labels table}
...
{
*   Find the unique labels table to use.  This is the first table that exists
*   from the current execution block to successively higher parents.  The top
*   execution block always has a unique labels table.
}
  ex_p := e.exblock_p;                 {init to look for labels table in current block}
  while ex_p^.ulab = nil do begin      {no unique labels table here ?}
